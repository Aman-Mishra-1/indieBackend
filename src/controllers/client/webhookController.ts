  ...
  async stripeWebhook(
    ...
    if (!sig) {
      ...
      res
        ...
        .send("Missing stripe-signature in header");
      return;
    }
      ...
      switch (event.type) {
        case "checkout.session.completed":
          ...
          if (!clientId || !freelancerId || !jobId) {
            ...
            res
              .status(HttpStatus.BAD_REQUEST)
              .send("Metadata missing in session");
            return;
          }

          console.log(
            `✅ Client ${clientId} paid for job ${jobId}, amount: ${paymentAmount}`
          );

          const platformFee = paymentAmount * 0.1;
          const freelancerEarning = paymentAmount - platformFee;

          const contract = await Contract.findOneAndUpdate(
            { clientId, jobId },
            { $set: { escrowPaid: true, status: "Started" } },
