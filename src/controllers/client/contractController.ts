...
export class ContractController implements IContractController {
    ...
    try {
      ...
      if (!jobId || !clientId || !freelancerId || !amount) {
        res
          ...
          .json({ message: Messages.MISSING_PARAMETERS });
        return;
      }
      const contract = await this._contractService.createContract(
        jobId,
        clientId,
        freelancerId,
        amount
      ...
      res
        ...
        .json({ message: Messages.CONTRACT_CREATE, contract });
    } catch (error) {
      next(error);
    }
  }
  ...
  async getClientContracts(
    ...
    try {
      ...
      const contracts = await this._contractService.getClientContracts(
        clientId
      );
      res
  ...
  async isContractExist(
    ...
    try {
      ...
      const contract = await this._contractService.isContractExist(
        jobId,
        clientId
      );
  ...
  async requestFundRelease(
    ...
    try {
      ...
      res
        .status(HttpStatus.OK)
        .json({ message: Messages.FUND_RELEASE_REQUEST });
    } catch (error) {
      next(error);
    }
  }
