"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReviewRepository = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
const reviewMode_1 = __importDefault(require("../../models/client/reviewMode"));
const baseRepository_1 = require("../base/baseRepository");
class ReviewRepository extends baseRepository_1.BaseRepository {
    constructor() {
        super(reviewMode_1.default);
    }
    createReview(reviewData) {
        return __awaiter(this, void 0, void 0, function* () {
            const review = new reviewMode_1.default(reviewData);
            return yield this.create(review);
        });
    }
    ;
    getReviewsByFreelancer(freelancerId) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.model.find({
                freelancerId: new mongoose_1.default.Types.ObjectId(freelancerId),
                isDeleted: false
            })
                .populate('clientId', 'name email profilePic')
                .populate('contractId')
                .sort({ createdAt: -1 });
        });
    }
    ;
    getReviewByContract(contractId) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.model.findOne({
                contractId: new mongoose_1.default.Types.ObjectId(contractId),
                isDeleted: false
            });
        });
    }
    ;
}
exports.ReviewRepository = ReviewRepository;
